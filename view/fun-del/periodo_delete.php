						...
						<div class="cardbox">
							<div class="cardbox-body">
								...
								if(isset($_GET['cod'])){
									...
									// SELECT tbl_periodo
									$estado_peri = 0;
									$sql_periodo = mysqli_query($con, "SELECT estado_peri FROM tbl_periodo WHERE cod_periodo = $cod_periodo");
									while ($row_p = mysqli_fetch_array($sql_periodo)) {
										if($row_p['estado_peri'] == 0)
											$estado_peri = 1;
									}
									// UPDATE tbl_periodo
									$update_periodo = mysqli_query($con, "UPDATE tbl_periodo SET estado_peri = $estado_peri 
										WHERE cod_periodo = $cod_periodo");
									if(mysqli_affected_rows($con) > 0){
										?>
										<div class="text-center">
											<h2 class="text-primary">Registro modificar.!</h2>
										</div>
										<?php
									}else{
										?>
