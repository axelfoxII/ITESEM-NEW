<?php
	...
	if (verificar_usuario()){
		...
		if (isset($_POST['suc']) && !isset($_POST['car']) && !isset($_POST['nom']) && !isset($_POST['ape'])) {
			$cod_suc = $_POST['suc'];
			$sql_estudiante = mysqli_query($con, "SELECT cod_estudiante, nombre_per, apellido_per, carnet_per, complemento_carnet_per, nombre_car, resolucion_ministerial_car, sigla_suc 
			...
			FROM tbl_estudiante, tbl_persona, tbl_carrera, tbl_sucursal 
			WHERE cod_sucursal = cod_sucursal_est AND cod_persona_est = cod_persona AND cod_carrera_est = cod_carrera AND estado_per = 1 
			AND nombre_per LIKE '%$nombre%' AND apellido_per LIKE '%$apellido%' AND cod_sucursal_est = $cod_suc 
			...
			<tbody>
				...
				while ($row_e = mysqli_fetch_array($sql_estudiante)) {
					...
					<tr>
						...
						<td data-title="MODIFICAR" align="center"><a href="estudiante_modificar.php?cod=<?php echo $row_e['cod_estudiante']; ?>">Modificar</a></td>
						<td data-title="ELIMINAR" align="center"><a href="fun-del/estudiante_delete.php?cod=<?php echo $row_e['cod_estudiante']; ?>" onclick="return confirm('¿Estás seguro de eliminar este registro?')">Eliminar</a></td>
					</tr>
				<?php } ?>
			</tbody>
			<?php
