		...
		if (mysqli_num_rows($sql_persona)) {
			...
			while ($row_p = mysqli_fetch_array($sql_persona)) {
				...
				if($row_p['complemento_carnet_per'] != "" && $row_p['complemento_carnet_per'] != NULL)
					...
					$sql_docente = mysqli_query($con, "SELECT cod_docente FROM tbl_docente WHERE cod_persona_doc = $cod_persona");
					if(mysqli_num_rows($sql_docente) > 0){
						?>
						<a class="text-danger"><h6><?php echo $row_p['carnet_per'].$complemento." - ".$row_p['nombre_per']." ".$row_p['apellido_per']; ?> - ** YA ESTÁ COMO DOCENTE **</h6></a>
						<?php
					}else{
						?>
						<a href="docente_nuevo.php?cod=<?php echo $row_p['cod_persona']; ?>"><h6><?php echo $row_p['carnet_per'].$complemento." - ".$row_p['nombre_per']." ".$row_p['apellido_per']; ?></h6></a>
						<?php
					}
			}
		}
