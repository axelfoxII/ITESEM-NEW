		...
		if(isset($_REQUEST['tipo'])){
			...
			switch ($tipo) {
				case 'nro_recibo':
					...
					if(mysqli_num_rows($sql) > 0){
						while ($row = mysqli_fetch_array($sql)) {
							$det_estado = "<div class='text-primary'>ACTIVO</div>";
							...
							$fecha_rec = date_format(date_create($row['fecha_venta']), "d-m-Y H:i");
							...
							<tr>
								...
								<td align="center"><a href="ingresos_caja_ver.php?cod=<?php echo $cod_venta; ?>">Ver Recibo</a></td>
				...
				case 'carnet':
					...
					$sql = mysqli_query($con, "SELECT cod_venta, monto_total_venta, nombre_per, apellido_per, carnet_per, fecha_venta, estado_venta FROM tbl_venta, tbl_estudiante, tbl_persona 
						WHERE carnet_per LIKE '$carnet%' AND cod_estudiante_venta = cod_estudiante AND cod_persona_est = cod_persona AND cod_sucursal_est = $cod_sucursal ORDER BY cod_venta");
					if(mysqli_num_rows($sql) > 0){
						while ($row = mysqli_fetch_array($sql)) {
							$det_estado = "<div class='text-primary'>ACTIVO</div>";
							if($row['estado_venta'] == 0)
